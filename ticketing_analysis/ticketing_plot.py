...
df_prompt = """
    You are a data-to-JSON converter.
    ...
    Task:
    Convert this raw SQL output (list of tuples) into a JSON object with exactly two keys:
    1. "columns" — a list of column names inferred from the data (for example: ["ClientId", "ClientName", "ClientEmail", "TicketCount"])
    2. "data" — the list of tuples as they appear in the raw data.

    Output Format (strict JSON, no markdown, no explanation):

    {{
    "columns": ["ClientId", "ClientName", "ClientEmail", "TicketCount"],
    "data": [
        (2226, "Worktual Admin", "worktualadmin@worktual.com", 33),
        (11769, "Waste King", "wasteking@worktual.com", 31)
    ]
    }}
"""
...
def data_frame(raw_data):
    ...
    llm = GptOSS()
    prompt = df_prompt.format(raw_data=raw_data)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        df = data_frame(raw_data = raw_data)
        print('-'*80)
        print(df)
